...
long double operator+(const U &x,const V &a)
{
	long double pay;
	pay = *x[position].Tax_Amount + *a[position].Tax_Amount;
	cout<<"Total Tax Amount of Both The Clients Are: "<<pay<<endl;
	return pay;
}
...
class Farmer : public Tax_Payer
{
	...
	public:
		...
		template<class U,class V>
		friend long double operator+(const U &x,const V &a);
	...
	public:
		...
		void TaxCollect()
		{
		    ...
		    while (true) {
	   			...
	   			cin.clear();
	   			cin.ignore(1000, '\n');
	   			cout<<"Enter Correct Value Please: ";
   			}
